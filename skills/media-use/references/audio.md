...
- **Request** `{ provider?, lang?, speed?, lines: [{ id, text, sfx?: [name | { name, offset_s, role? }] }], bgm: { mode?, query?, prompt? } }`: `id` joins each line back to your model; string SFX remain valid at offset 0; `bgm.mode` = `retrieve | generate | none` (omit for auto). `--only tts,bgm,sfx` runs a subset and merges into an existing `--out`.
